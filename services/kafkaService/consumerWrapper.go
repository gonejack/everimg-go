...
type consumerWrapperType struct {
	clusterConfig   *viper.Viper
	topicConsumers  sync.Map
	topicSubsQueues sync.Map
}

func (cw *consumerWrapperType) subscribe(groupId string, topic string) (queue chan []byte) {
...
func (cw *consumerWrapperType) unsubscribe(unSubscribeQueue chan []byte) (ok bool) {
...
func (cw *consumerWrapperType) readThread(consumer *cluster.Consumer, topic string) {
	for {
		select {
...
func (cw *consumerWrapperType) Close() {
...
func NewConsumerWrapper(conf *viper.Viper) (cw *consumerWrapperType) {
	cw = &consumerWrapperType{
		clusterConfig: conf,
	}
